...
std::shared_ptr<Heightmap> ExtractTile(GDALDataset* dataset, int zoom, int x, int y)
{
    ...
    if (xOff + xSize > datasetWidth) {
        xSize = datasetWidth - xOff;
    }
    if (yOff + ySize > datasetHeight) {
        ySize = datasetHeight - yOff;
    }
...
bool BuildTile(GDALDataset* dataset,
               ...
               float maxError)
{
    ...
    if (!hm || hm->Width() * hm->Height() == 0) {
        std::cerr << "Failed to extract tile " << zoom << "/" << x << "/" << y
                  << std::endl;
        return false;
    }
    ...
    for (size_t i = 0; i < points2D.size(); ++i) {
        ...
        if (p.x == 0)
            westIndices.push_back(i);
        if (p.y == 0)
            southIndices.push_back(i);
        if (p.x == width - 1)
            eastIndices.push_back(i);
        if (p.y == height - 1)
            northIndices.push_back(i);
    }
    ...
    const std::string tileDir =
        outputDir + "/" + std::to_string(zoom) + "/" + std::to_string(x);
...
int main(int argc, char** argv)
{
    ...
    // Open input dataset
    GDALDataset* dataset =
        static_cast<GDALDataset*>(GDALOpen(inputPath.c_str(), GA_ReadOnly));
    if (!dataset) {
        std::cerr << "Failed to open input file: " << inputPath << std::endl;
        return 1;
    }

    std::cout << "Dataset: " << dataset->GetRasterXSize() << " x "
              << dataset->GetRasterYSize() << std::endl;
    ...
    for (int zoom = minZoom; zoom <= maxZoom; ++zoom) {
        ...
        std::cout << "Generating zoom level " << zoom << " (" << tilesPerSide << " x "
                  << tilesPerSide << " tiles)" << std::endl;
        ...
        for (int y = 0; y < tilesPerSide; ++y) {
            for (int x = 0; x < tilesPerSide; ++x) {
                ...
                if (processed % 10 == 0 || processed == totalTiles) {
                    std::cout << "  Progress: " << processed << "/" << totalTiles << "\r"
                              << std::flush;
                }
            }
        }
        ...
        std::cout << std::endl;
        std::cout << "  Generated " << successCount << "/" << totalTiles << " tiles"
                  << std::endl;
    }
